...
the input :class:`~nirconn.core.timeseries.NirsTimeSeries` carries ``sfreq``.
"""
...
class Coherence(ConnectivityEstimator):
    ...
    surrogates:
        ...
        Fourier phase-randomised surrogates (the empirical null for coherence;
        the band-averaged MSC has no simple analytic test). Default ``None`` --
        no p-values.
    surrogate_seed:
