...
* coercing arbitrary input into a :class:`~nirconn.core.timeseries.NirsTimeSeries`,
* iterating over chromophores (so a method author never writes that loop),
* validating shapes,
* wrapping raw NumPy output in labelled :class:`~nirconn.core.result.ConnectivityMatrix`
  / :class:`~nirconn.core.result.ConnectivityResult` objects with full provenance.
...
class ConnectivityEstimator(ABC):
    ...
    #: Tail of the surrogate test for methods that support one. Connectivity
    #: magnitudes (PLV, coherence, ...) are non-negative, so the default is the
    #: one-sided upper tail.
    surrogate_tail: ClassVar[str] = "greater"
    ...
    def _surrogate_pvalues(self, x: np.ndarray, statistic) -> np.ndarray | None:
        """Phase-randomised surrogate p-values, if the estimator requested them.

        A method opts in by accepting ``surrogates`` (and optional
        ``surrogate_seed``) in ``__init__`` and storing them on ``self``, then
        passing its pure statistic-computing function here. Returns ``None`` when
        ``surrogates`` is unset, so callers can fall back to an analytic p-value.
        """
        n_surr = getattr(self, "surrogates", None)
        if not n_surr:
            return None
        from ..stats.surrogates import surrogate_pvalues

        seed = getattr(self, "surrogate_seed", None)
        _, p = surrogate_pvalues(
            statistic,
            x,
            n_surrogates=int(n_surr),
            rng=seed,
            tail=self.surrogate_tail,
        )
        return p
