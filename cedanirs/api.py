...
def connectivity(
    ...
    **params,
    ...
    ----------
    data:
        A :class:`~cedanirs.core.timeseries.NirsTimeSeries`, a NumPy array
        (2-D ``(channel, time)`` or 3-D ``(chromophore, channel, time)``), or an
        :class:`xarray.DataArray`.
    method:
        Registered estimator name (see
        :func:`~cedanirs.core.registry.list_estimators`). Defaults to
        ``"pearson"``.
    ...
    **params:
    ...
    -------
    ConnectivityResult
        One :class:`~cedanirs.core.result.ConnectivityMatrix` per chromophore.
    """
