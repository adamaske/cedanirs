...
:class:`~cedanirs.core.result.ConnectivityMatrix`/``ConnectivityResult`` or a
group-level :class:`~cedanirs.stats._results.GroupStatResult`.
...
def _panel_banner(ax, matrix, stat, alpha, correction, title, primary):
    ...
    bits += [f"alpha: {alpha}", f"correction: {correction}", f"cedanirs {__version__}"]
