...
* :class:`Study` -- the user-facing collection. It holds each subject's
  :class:`~cedanirs.core.result.ConnectivityResult` plus a single metadata table
...
The statistical tests themselves live in :mod:`cedanirs.stats.group`; the
...
class Study:
    ...
    Add each subject's :class:`~cedanirs.core.result.ConnectivityResult` with
    ...
    ::
        ...
        study = cedanirs.Study(name="rest-vs-task")
        for sid, rec in recordings.items():
            res = cedanirs.connectivity(rec, method="pearson")
